...
##----------------------------------------------------------------------------
# plot bias
plot(bias_simMLE[[1]] ~ theta, type='l', col=1, lwd=2, 
     ylab="BIAS",
     xlab="Proficiency")
lines(bias_anal1 ~ theta, col=2, lwd=2)
lines(bias_simEOS[[1]] ~ theta, col=3, lwd=3)
legend("topright", legend=c("Simulation", "Analytic"), col=1:2, lwd=2)

# plot csee
plot(csee_simEOS[[3]] ~ theta, type='l', col=1, lwd=2,
     ylim=c(0, max(csee_simMLE[[1]])),
     ylab="CSEE",
     xlab="Proficiency")
lines(csee_simMLE[[3]] ~ theta, col=2, lwd=2)
lines(csee_anal1 ~ theta, col=3, lwd=2)
lines(csee_anal2 ~ theta, col=4, lwd=2)
legend("top", legend=c("MLE (ENC)", "MLE (MLE)", "Anal (ENC)", "Anal (MLE)"), 
       col=1:4, lwd=2)
