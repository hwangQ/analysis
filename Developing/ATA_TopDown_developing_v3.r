...
ata_mst <- function(item.pool, constraints, target.dens, target.max, relative=FALSE, rel.tol=c(0.7, -0.5),
	...
	# 		add.constraint(lprec=sim_mod, xt=c(info.RDP[, m], -info.RDP[, m]), type="=", rhs=0, indices=indices)
	# 	}
	# }
	...
	# 			for(j in nopick.num) {
	# 				index.1 <- (unique(pathway[, s])[pick.num] * I - I + 1):(unique(pathway[, s])[pick.num] * I)
	# 				index.2 <- (unique(pathway[, s])[j] * I - I + 1):(unique(pathway[, s])[j] * I)
	# 				indices <- c(index.1, index.2)
	# 				add.constraint(lprec=sim_mod, xt=c(info_list[[pick.num]][, i], -info_list[[j]][, i]), type=">=", rhs=0, indices=indices)
	# 			}
	# 		}
	# 	}
	# }
...
plot(info_path1 ~ theta, ylim=c(0, 28), type='l', col=1)
