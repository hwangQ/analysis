...
bank_info <- rbind(bank_info, bank_info)
...
ata_mst <- function(item.pool, constraints, target.dens, target.max, relative=FALSE, rel.tol=c(0.7, -0.5),
	...
	Nc_stg3 <- cont_require[, 4]
	Nc_path <- cont_require[, 5]
	...
	theta.1 <- seq(end.post[1], mid.post[1], length.out=5)
	theta.2 <- seq(mid.post[1], mid.post[2], length.out=5)
	theta.3 <- seq(mid.post[2], end.post[2], length.out=5)
	info.1 <- test.info(x=df_bank, theta=theta.1, D=D)$itemInfo
	info.2 <- test.info(x=df_bank, theta=theta.2, D=D)$itemInfo
	info.3 <- test.info(x=df_bank, theta=theta.3, D=D)$itemInfo
	...
	for(w in 1:n.pathway) {
		add.constraint(lprec=sim_mod, xt=rep(1, I * n.stage), type="=", rhs=test.length, indices=c(1:I, (pathway[w,2]*I-I + 1):(pathway[w,2]*I), (pathway[w,3]*I-I + 1):(pathway[w,3]*I)))
	}

	# constraint: content category
	for(w in 1:n.pathway) {
		for(i in 1:length(Nc_path)) {
			add.constraint(lprec=sim_mod, xt=rep(1, length(Vc[[i]]) * n.stage), type=">=", rhs=Nc_path[i], indices=c(Vc[[i]], (pathway[w,2]*I-I + Vc[[i]]), (pathway[w,3]*I-I + Vc[[i]])))
		}
	}
	...
	for(w in 1:n.pathway) {
		for(i in 1:I) {
			add.constraint(lprec=sim_mod, xt=rep(1, 3), type="<=", rhs=1, indices=c(i, (pathway[w,2]*I-I)+i, (pathway[w,3]*I-I)+i))
		}
	}

	min.size <- test.length * 0.2
	# constraint: minimum module length
	for(f in 1:F) {
		add.constraint(lprec=sim_mod, xt=rep(1, I), type=">=", rhs=min.size, indices=(I*(f-1)+1):(I*f))
	}
		...
		for(w in 1:n.pathway) {
			if(w %in% group[[1]]) {
				for(i in 1:length(theta.1)) {
					add.constraint(lprec=sim_mod, xt=c(rep(info.1[, i], n.stage), -1), type=">=", rhs=0, indices=c(1:I, (pathway[w,2]*I-I + 1):(pathway[w,2]*I), (pathway[w,3]*I-I + 1):(pathway[w,3]*I), M))
				}
			}
		}
		...
		for(w in 1:n.pathway) {
			...
			if(w %in% group[[2]]) {
				for(i in 1:length(theta.2)) {
					add.constraint(lprec=sim_mod, xt=c(rep(info.2[, i], n.stage), -1), type=">=", rhs=0, indices=c(1:I, (pathway[w,2]*I-I + 1):(pathway[w,2]*I), (pathway[w,3]*I-I + 1):(pathway[w,3]*I), M))
				}
			}
			if(w %in% group[[3]]) {
				for(i in 1:length(theta.3)) {
					add.constraint(lprec=sim_mod, xt=c(rep(info.3[, i], n.stage), -1), type=">=", rhs=0, indices=c(1:I, (pathway[w,2]*I-I + 1):(pathway[w,2]*I), (pathway[w,3]*I-I + 1):(pathway[w,3]*I), M))
				}
			}
		}
	...
	for(i in (i+1):(i+nmod[2])) {
		assign(paste0("info_stg3_", i-nmod[1]), test.info(x=df_sim_list[[i+1]], theta=theta, D=D)$testInfo)
	}
